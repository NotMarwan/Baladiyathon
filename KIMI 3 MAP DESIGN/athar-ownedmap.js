/**
 * أثر — الخريطة المملوكة (نسخة «غرفة التحكم»)
 * ---------------------------------------------------------------------------
 ...
 * UMD بنفس نمط athar-engine.js — مسارات Node لا تعتمد Leaflet.
 */
(function (root, factory) {
  ...
  function makeLegendControl(L) {
    ...
    legend.onAdd = function () {
      var div = L.DomUtil.create('div', 'athar-legend');
      div.innerHTML =
        '<div class="athar-legend-toggle"><span>دليل الخريطة</span><span class="athar-legend-chevron">▾</span></div>' +
        '<div class="athar-legend-body">' +
        '<div class="athar-legend-row"><span class="athar-swatch" style="background:#e8a33d"></span>طرق سريعة</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch" style="background:#d18a4a"></span>طرق شريانية</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch" style="background:#7fb3d1"></span>طرق رئيسية</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch" style="background:#5b7a94"></span>طرق ثانوية وفرعية</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch" style="background:#59d6f2"></span>الممر — طريق الملك فهد</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch dashed" style="color:#d19231"></span>مسار بديل محسوب</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch dot" style="background:#e5484d"></span>موقع الحفر / إغلاق</div>' +
        '<div class="athar-legend-row"><span class="athar-swatch dot" style="background:#34d399"></span>انسياب بعد الجدولة</div>' +
        '</div>';
      L.DomEvent.disableClickPropagation(div);
      L.DomEvent.disableScrollPropagation(div);
      var toggle = div.querySelector('.athar-legend-toggle');
  ...
  function load(map, geojson, L, onRoadClick) {
    ...
    var glowRenderer = L.canvas({ pane: 'athar-glow', padding: 0.5 });
    var rendererA = L.canvas({ pane: 'athar-r1', padding: 0.5 });
    var rendererB = L.canvas({ pane: 'athar-r2', padding: 0.5 });
    var rendererC = L.canvas({ pane: 'athar-r3', padding: 0.5 });
    ...
    var labelLayers = labelDefs.map(function (def) {
      ...
      var tt = L.tooltip({
        ...
        direction: 'center',
        interactive: false,
        className: 'athar-road-label' + (def.cls === 'motorway' ? ' athar-label-major' : ''),
      });
    ...
    var vignette = L.DomUtil.create('div', 'athar-vignette');
    ...
    makeTextControl(L, 'topright', 'athar-demo-chip', 'بيانات توضيحية للعرض', 'كل الأرقام المرورية على هذه الخريطة توضيحية للعرض').addTo(map);
    makeTextControl(L, 'topright', 'athar-north', 'ش', 'شمال').addTo(map);

    // --- دخول متدرج: السريع أولًا ثم الرئيسي ثم الفرعي (مرة واحدة) ---
    ['athar-glow', 'athar-r1', 'athar-r2', 'athar-r3'].forEach(function (paneName) {
      var pane = map.getPane(paneName);
      if (pane) pane.classList.add('athar-enter');
    });
    setTimeout(function () {
      ['athar-glow', 'athar-r1', 'athar-r2', 'athar-r3'].forEach(function (paneName) {
        var pane = map.getPane(paneName);
        if (pane) pane.classList.remove('athar-enter');
      });
    }, 2200);
