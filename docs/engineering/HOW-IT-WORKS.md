...
## Fable-as-Manager: طبقة التفويض (أُضيفت بتاريخ 2026-07-09)

بناءً على حل Anthropic لتكلفة Fable 5 (the-decoder, 2026): Fable يتوقف عن عمل كل شيء بنفسه ويدير موديلات أرخص بدلاً من ذلك. نمطان مثبتان:
- **Orchestrator (المنسّق)** — Fable يخطّط ← عمّال Sonnet 5 ينفّذون ← Fable يراجع ويجمّع: ≈96% من جودة Fable-solo بـ ≈46% من التكلفة.
- **Advisor (المستشار)** — Sonnet 5 ينفّذ ويستشير Fable فقط عند التعثّر: ≈92% بـ 63%.

### كيف ينفّذ هذا المستودع النمط
`fable-orchestrator/manager.py` هو المنسّق (orchestrator):
1. **plan (التخطيط)** — Fable 5 يقسّم المهمة إلى 2–5 مهام فرعية، كل واحدة موسومة بـ `scout` (بحث) أو `worker` (بناء).
2. **execute (التنفيذ)** — `scout` ← Haiku 4.5 ($1/$5 لكل MTok)، `worker` ← Sonnet 5 ($2/$10 سعر تمهيدي حتى 2026-08-31). رفض السلامة يُعاد المحاولة مرة واحدة على Opus 4.8.
3. **review (المراجعة)** — Fable يفحص كل نتيجة بجهد منخفض؛ يرسل تصحيحاً واحداً إذا لزم.
4. **synthesize (التجميع)** — Fable يجمّع كل شيء في الإجابة النهائية.
5. **report + log (التقرير والسجل)** — تكاليف التوكن الحقيقية لكل موديل تُطبع وتُحفظ في `logs/fable-run-*.md`.

الاستخدام: `python fable-orchestrator/manager.py "مهمتك هنا"`

وكلاء WezTerm الأربعة (agent1–4) يتبعون نفس السياسة — راجع قسم "Model & Delegation Policy" في كل `agentN/AGENTS.md`.
