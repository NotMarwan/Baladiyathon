# Masar Innovation Loops Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build five offline, testable decision loops above the existing Masar BPR engine so the innovation is demonstrated through dynamic boundaries, corridor impact budgets, quantitative ranking explanations, multi-permit conflict detection, and post-work calibration.

**Architecture:** Keep `masar-engine.js` unchanged and consume it from a new offline laboratory page. Each innovation loop is a small UMD module that works in both Node and a browser opened from `file://`; a single UMD data fixture supplies clearly labeled illustrative inputs, and a plain `node:assert` suite verifies every public contract.
...
- Create new files only; do not modify `masar-engine.js`, existing HTML pages, `engine-test.js`, or `server.js`.
...
- Create `presentation/masar-boundary.js`: pure dynamic impact-boundary calculation.
- Create `presentation/masar-budget.js`: pure monthly corridor budget decision and next-window search.
- Create `presentation/masar-reasons.js`: pure data-driven candidate ranking and contribution explanation.
- Create `presentation/masar-conflict.js`: pure N-permit temporal/spatial matrix, derived interaction ratio, and Dig-Once groups.
- Create `presentation/masar-memory.js`: pure prediction/observation evaluation and ratio-of-totals calibration.
- Create `presentation/tests/innovation-test.js`: one dependency-free test harness covering fixtures and every module.
- Create `presentation/masar-lab.html`: offline interactive laboratory with one scene per module.
...
- Create: `presentation/masar-boundary.js`

**Interfaces:**
- Produces: `MasarInnovationDemoData` with `meta`, `boundary`, `budget`, `permits`, `ranking`, and `memoryRecords`.
- Produces: `MasarBoundary.calculate(input) -> {radiusMeters, baseRadiusMeters, demandRatio, residualCapacityRatio, components, method, provenance}`.
...
(function (root, factory) {
  ...
  else root.MasarInnovationDemoData = value;
...
```powershell
node presentation/tests/innovation-test.js
git add presentation/data/innovation-demo-data.js presentation/tests/innovation-test.js presentation/masar-boundary.js
...
- Create: `presentation/masar-budget.js`

**Interfaces:**
- Consumes: permits with `corridorId`, `start`, `end`, `delayVehHours`, and `status`.
- Produces: `MasarBudget.assess(request, existingPermits, options) -> {decision, usedBefore, projectedUsage, remainingAfter, utilizationPct, suggestedStart, derivation, provenance}`.
...
Expected: failure containing `Cannot find module '../masar-budget.js'`.
...
git add presentation/tests/innovation-test.js presentation/masar-budget.js
...
**Files:**
- Modify: `presentation/tests/innovation-test.js`
- Create: `presentation/masar-reasons.js`

**Interfaces:**
- Consumes: a baseline and candidates containing four loss metrics with lower values preferred.
- Produces: `MasarReasons.explain(baseline, candidates, provenance) -> {ranked, method, provenance}` where every ranked candidate has numeric `factors`, `totalPoints`, `rank`, and factor `weightPct`.
...
Expected: failure containing `Cannot find module '../masar-reasons.js'`.
...
node presentation/tests/innovation-test.js
node presentation/tests/engine-test.js
git add presentation/tests/innovation-test.js presentation/masar-reasons.js
...
**Files:**
- Modify: `presentation/tests/innovation-test.js`
- Create: `presentation/masar-conflict.js`

**Interfaces:**
- Produces: `MasarConflict.analyze(permits, options) -> {permits, matrix, conflicts, digOnceGroups, interactionRatio, interactionDerivation, provenance}`.
...
Expected: failure containing `Cannot find module '../masar-conflict.js'`.
...
node presentation/tests/innovation-test.js
node presentation/tests/engine-test.js
git add presentation/tests/innovation-test.js presentation/masar-conflict.js
...
**Files:**
- Modify: `presentation/tests/innovation-test.js`
- Create: `presentation/masar-memory.js`

**Interfaces:**
- Produces: `MasarMemory.calibrate(records, options) -> {records, correctionFactor, beforeMapePct, afterMapePct, casesImproved, casesWorsened, verdictCounts, method, provenance}`.
...
Expected: failure containing `Cannot find module '../masar-memory.js'`.
...
node presentation/tests/innovation-test.js
node presentation/tests/engine-test.js
git add presentation/tests/innovation-test.js presentation/masar-memory.js
...
**Files:**
- Create: `presentation/masar-lab.html`
- Modify: `presentation/tests/innovation-test.js`

**Interfaces:**
- Consumes all six UMD globals plus unchanged `window.MasarEngine`.
...
['masar-engine.js', 'masar-boundary.js', 'masar-budget.js', 'masar-reasons.js', 'masar-conflict.js', 'masar-memory.js', 'innovation-demo-data.js']
...
Expected: failure containing `ENOENT` for `masar-lab.html`.
...
<script src="./masar-boundary.js"></script>
<script src="./masar-budget.js"></script>
<script src="./masar-reasons.js"></script>
<script src="./masar-conflict.js"></script>
<script src="./masar-memory.js"></script>
```

The inline controller must:

```js
const Demo = window.MasarInnovationDemoData;
function renderAll() {
  renderBoundary(MasarBoundary.calculate(readBoundaryInputs()));
  renderBudget(MasarBudget.assess(readRequestedPermit(), Demo.permits, readBudgetOptions()));
  renderReasons(MasarReasons.explain(readRankingBaseline(), readRankingCandidates(), Demo.meta));
  renderConflict(MasarConflict.analyze(readPermits(), { coordinationWindowHours: Number(document.querySelector('#coordination-hours').value), provenance: Demo.meta }));
  renderMemory(MasarMemory.calibrate(readMemoryRecords(), { successThresholdPct: Number(document.querySelector('#success-threshold').value), provenance: Demo.meta }));
}
document.querySelectorAll('input, select').forEach((element) => element.addEventListener('input', renderAll));
renderAll();
```

Each scene must show its equation or derivation beside the output, a source/assumption badge, and a user-editable input that visibly changes the result. The conflict scene must draw the full N×N matrix; the memory scene must show successful and failed cases plus improved and worsened cases; the competitive-position copy must say that Singapore and one.network prove adjacent capabilities while Masar differentiates through the Saudi combination, not primacy.
...
```powershell
git add presentation/masar-lab.html presentation/tests/innovation-test.js
git commit -m "feat: showcase five Masar innovation loops"
```
...
```powershell
rg -n "الأول|الوحيد|غير مسبوق|https?://" presentation/masar-*.js presentation/masar-lab.html presentation/data/innovation-demo-data.js REPORT.md
rg -n "افتراض توضيحي للعرض|src-003|src-021|src-024|src-030" presentation/masar-lab.html presentation/data/innovation-demo-data.js REPORT.md
```
...
```powershell
git add REPORT.md docs/superpowers/plans/2026-07-23-masar-innovation-loops.md presentation/masar-boundary.js presentation/masar-budget.js presentation/masar-reasons.js presentation/masar-conflict.js presentation/masar-memory.js presentation/masar-lab.html presentation/data/innovation-demo-data.js presentation/tests/innovation-test.js
