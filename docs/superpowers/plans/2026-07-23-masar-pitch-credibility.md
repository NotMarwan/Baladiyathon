# Masar Pitch Credibility Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Raise the Masar presentation criterion from 5.5/10 to at least 9.0/10 by making every claim honest and traceable, rebuilding the three-minute story, adding the required experiment and three-tier content, and delivering an adversarial self-review.
...
- Work only in `C:\Users\wasan\Downloads\Swarm\GPT 1\masar-crit6-pitch` on branch `crit6-pitch`.
- Modify only `presentation/masar-pitch.html`, `presentation/masar-merged.html`, `presentation/masar.html`, new presentation HTML pages, `REPORT.md`, this plan, and the ignored progress ledger required by the execution workflow.
- Never modify `presentation/masar-engine.js`, `presentation/masar-prototype.html`, `tests/`, or `server.js`.
...
- Competitor evidence must state Singapore’s `200–300` daily roadworks from `src-030` and the two-month advance-notice rule from `src-029`, then differentiate Masar without denying those strengths.
...
**Files:**
- Create: `presentation/masar-sources.html`
...
$html = Get-Content -Raw -Encoding utf8 -LiteralPath 'presentation\masar-sources.html'
$missing = $required | Where-Object { $html -notmatch ('id="' + [regex]::Escape($_) + '"') }
if ($missing) { throw "Missing source anchors: $($missing -join ', ')" }
```

Expected: FAIL because `presentation\masar-sources.html` does not exist.
...
<main>
  ...
  <section class="grid" id="sources">
    <article id="src-001" data-search="كود الطرق السعودي الدرجات الثلاث">
      ...
      <p>يوفّر الكود مرجعاً لثلاث درجات من فحص الحركة. يستعير «مسار» منطق التدرّج، ولا يقدمه كحدود ملزمة لتصاريح الحفر.</p>
    ...
    <article id="src-004" data-search="انبعاثات كود الطرق السعودي">
      ...
      <p>يربط الكود عوامل الانبعاث بنوع المركبة والحجم، لذلك يعرض «مسار» الانبعاثات كقياس فيزيائي أولاً.</p>
    ...
    <article id="src-008" data-search="بلدي الخريطة الحفريات التنسيق">
      ...
      <p>تعرض بلدي مسارات الحفريات المصرح بها وتدعم التنسيق المستقبلي والمتعدد؛ «مسار» طبقة قرار تكملها ولا تستبدلها.</p>
    ...
    <article id="src-015" data-search="الرياض 67 دقيقة التنقل">
      ...
      <p class="limit">حد الاستخدام: سياق حضري عام؛ لا ينسب إلى الحفريات ولا يمثل نتيجة لـ«مسار».</p>
    ...
    <article id="src-026" data-search="Aimsun محاكاة تنبؤ">
      ...
      <p class="limit">حد الاستخدام: معيار مستقبلي؛ لا يمثل قدرة حالية في «مسار».</p>
    ...
    <article id="src-030" data-search="سنغافورة 200 300 أعمال طرق يومياً ذكاء اصطناعي">
      ...
      <p class="limit">حد الاستخدام: سابقة منافسة قوية؛ تميّز «مسار» يجب أن يكون في الأثر الشبكي والاقتصاد والمعايرة.</p>
    ...
    <article id="src-031" data-search="Street Manager أكثر من 2 مليون أعمال سنوياً">
      ...
      <p class="limit">حد الاستخدام: يثبت قابلية التنسيق الرقمي على نطاق واسع، لا أثر «مسار» ولا حجم التصاريح السعودي.</p>
...
```powershell
git add presentation/masar-sources.html
git commit -m "feat: add unified Masar evidence ledger"
```
...
**Files:**
- Modify: `presentation/masar-pitch.html`
- Modify: `presentation/masar-merged.html`
- Modify: `presentation/masar.html`
...
```powershell
rg -n -i -- '2\.5M|150|300|659K|234K|19 خدمة|90%|فالنموذج مضمون|كلها مفتوحة|مقطع SUMO متحرك|ممر SUMO حي|يقيس جدواها|يتحسن مع كل تصريح|sciencedirect\.com|publications\.parliament|permits\.shtml|627' presentation/masar-pitch.html presentation/masar-merged.html presentation/masar.html
```
...
```html
<div class="ref"><a href="masar-sources.html#src-040">src-040 · بحث محكّم 2024 · يلزم إدخال محلي</a></div>
```
...
```html
<div class="ref"><a href="masar-sources.html#src-031">src-031 · وزارة النقل البريطانية · +2M عمل/سنة · ليس حجماً سعودياً</a></div>
```
...
```powershell
$files = 'presentation\masar-pitch.html','presentation\masar-merged.html','presentation\masar.html'
...
```powershell
git add presentation/masar-pitch.html presentation/masar-merged.html presentation/masar.html
...
```powershell
$html = Get-Content -Raw -Encoding utf8 -LiteralPath 'presentation\masar-pitch.html'
...
```html
<!-- SPEAKER NOTES: [0:00–0:15] اليوم يرى ضابط الترخيص موقع الحفر ومدته، لكنه لا يرى تكلفة القرار على حركة الناس قبل التوقيع. «مسار» يضيف هذا القرار قبل الموافقة. -->
<section class="slide cover" id="slide-1">
  <div class="slide-inner">
    ...
    <h1 style="justify-content:center">مسار <span class="latin">Masar</span></h1>
...
<section class="slide" id="slide-2">
  <div class="slide-inner">
    ...
    <h2>بلدي ينسّق الحفر؛ «مسار» يضيف قرار الحركة قبل الموافقة</h2>
    <p class="thesis">بلدي يملك خريطة للحفريات وتنسيقاً للمشاريع المستقبلية والمتعددة. الدليل العام لا يظهر تنبؤاً مرورياً منسوباً للحفرية أو ترتيباً كمياً للبدائل. «مسار» طبقة قرار تكمل هذا المسار.</p>
    <div class="ref"><a href="masar-sources.html#src-008">src-008 · لا يثبت غياب قدرات داخلية غير منشورة</a></div>
  </div>
</section>
...
<section class="slide" id="slide-3">
  <div class="slide-inner">
    ...
    <div class="ref"><a href="masar-sources.html#src-001">src-001 · منطق تدرّج مستعار من الكود؛ ليس حدود تصريح ملزمة</a></div>
  </div>
</section>
    ...
    <div class="demo-layout">
      <div class="demo-col">
        ...
        <div class="fallback-row">
          <a class="linkbtn" href="masar-prototype.html" target="_blank" rel="noopener">افتح النموذج في نافذة كاملة ↗</a>
        </div>
      </div>
      <div class="device-frame">
        <div class="device-bar">
          <span class="device-dot r"></span><span class="device-dot y"></span><span class="device-dot g"></span>
          <span class="device-url">masar-prototype.html</span>
        </div>
        <div class="device-screen">
          <iframe data-src="masar-prototype.html" loading="lazy" title="نموذج مسار الحالي"></iframe>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="slide-inner">
    ...
    <div class="stats">
      <div class="stat">
        ...
        <div class="ref"><a href="masar-sources.html#src-031">src-031 · وزارة النقل البريطانية · ليس حجماً سعودياً ولا نتيجة لأثر</a></div>
      </div>
    </div>
  </div>
</section>
...
<!-- SPEAKER NOTES: [2:40–3:00] «مسار» ليس منصة تصاريح جديدة؛ هو طبقة قرار سعودية حول بلدي، تبدأ بفحص خفيف اليوم وتكبر بالدليل والمعايرة. المطلوب: تجربة ظل وبيانات مصرح بها لاختبار الدقة. -->
  ...
  <div class="slide-inner">
    ...
    <div class="stats">
      <div class="stat">
        ...
        <div class="ref"><a href="masar-sources.html#src-030">src-030 · هيئة النقل البري في سنغافورة · 2025</a></div>
      </div>
    </div>
    <p class="limit">تميّز «مسار»: أثر شبكي منسوب، اقتصاد وقت وفق منهج سعودي، بدائل زمنية مفسّرة، ومعايرة بعد التنفيذ. لا ندّعي أن مراجعة الوثائق بالذكاء الاصطناعي ابتكارنا.</p>
  </div>
</section>
  ...
  <div class="slide-inner">
    ...
    <div class="stats">
      <div class="stat">
        ...
        <div class="ref"><a href="masar-sources.html#src-029">src-029 · تحديث 2026 · القاعدة خاصة بسنغافورة وتدخل لاحقاً</a></div>
      </div>
    </div>
    <p class="limit">تميّز «مسار»: لا يكتفي باستلام الجدول مبكراً؛ يقارن بدائله على حركة الناس ويختبر دقة التوقع بعد التنفيذ.</p>
  </div>
</section>
```
...
```powershell
git add presentation/masar-pitch.html
git commit -m "feat: rebuild Masar three-minute story"
```
...
foreach ($file in $files) {
  $html = Get-Content -Raw -Encoding utf8 -LiteralPath $file
  if ($html -notmatch 'masar-sources\.html') { throw "$file does not link the evidence page" }
...
<section>
  ...
  <div class="ref"><a href="masar-sources.html#src-001">src-001 · منطق تدرج مستعار من الكود، وليس حدود تصريح ملزمة</a></div>
</section>
```
...
<section>
  ...
  <div class="ref"><a href="masar-sources.html#src-001">src-001 · تكييف منهجي يحتاج اعتماد الجهة</a></div>
</section>
```
...
<p><a href="masar-sources.html">افتح سجل الأدلة الكامل ←</a></p>
```

Keep only source-list items whose links are present in `masar-sources.html`; remove Wikipedia and stale Parliament/ScienceDirect/FHWA entries.
...
```powershell
$files = 'presentation\masar-pitch.html','presentation\masar-merged.html','presentation\masar.html','presentation\masar-sources.html'
foreach ($file in $files) {
  $html = Get-Content -Raw -Encoding utf8 -LiteralPath $file
...
```markdown
# تقرير معيار العرض التقديمي — مسار
...
|---|---|---|---|
| presentation/masar-sources.html | جديد | لا توجد واجهة موحدة | سجل أدلة قابل للبحث بمعرفات وحدود استخدام وروابط رسمية |
| presentation/masar-pitch.html | القصة الرئيسية | 10 شرائح وملحق منافسين يقطع المسار | 8 شرائح رئيسية موقوتة حتى 3:00 ثم ملاحق |
| presentation/masar-pitch.html | الأرقام | 2.5M و150/300 وأرقام غير مسجلة | +2M من src-031 وحذف الأرقام غير المسجلة ووسم أرقام التجربة |
| presentation/masar-pitch.html | المنافسون | قدرات عامة بلا أقوى أرقام سنغافورة | 200–300 يومياً من src-030 وشهران من src-029 مع تميّز صادق |
| presentation/masar-pitch.html | العمق | لا شريحة تجربة ولا درجات ثلاث | تجربة ظل ودرجات تحليل ثلاث وملاحظات متحدث |
| presentation/masar-merged.html | القدرات | SUMO حي وBalady API مفتوحة ونموذج مضمون | قدرة حالية محددة وخارطة طريق صريحة وربط بلدي مشروط |
| presentation/masar.html | الملخص | SUMO وقدرات شبكية كأنها حالية | درجة تحليلية حالية ومسار توضيحي وتكامل متخصص لاحق |
| الملفات الثلاثة | المصادر | روابط متفرقة ومنحرفة | روابط موحدة إلى masar-sources.html ومعرفات السجل |
...
```powershell
git add presentation/masar-merged.html presentation/masar.html REPORT.md
