...
**Goal:** تحويل مخرجات محرك «أثر» من أرقام نقطية بساعات-مركبة إلى نطاقات ساعات-أشخاص وقيمة وقت وانبعاثات وأثر حافلات بمصادر سعودية رسمية، وإضافة تصدير WZDx وبطاقة حالة مقابلة إلى النموذج.

**Architecture:** كل الحسابات الجديدة دوال نقية تُضاف إلى `athar-engine.js` (UMD، بلا DOM/fetch/Date.now في الرياضيات) وتُختبر في `tests/engine-test.js` بنمط node:assert القائم. الواجهة `athar-prototype.html` تستهلك الدوال فقط وتعرضها بشارات مصدر. لا يُلمس أي ملف من ملفات خطة ChatGPT.

**Tech Stack:** Vanilla JS (UMD engine) · node:assert tests · Leaflet (مضمّن محلياً) · بلا أي تبعية جديدة.

## Global Constraints

- **تقسيم الملفات (صارم):** هذه الخطة تعدّل فقط: `presentation/athar-engine.js` · `presentation/athar-prototype.html` · `presentation/tests/engine-test.js`. ممنوع لمس `athar-pitch.html` / `athar-merged.html` / `athar.html` (ملكية ChatGPT).
...
|---|---|
| `presentation/athar-engine.js` | إضافة 6 دوال نقية: `personHours` · `timeValueSAR` · `co2Range` · `transitImpact` · `wzdx` · `predictionError` + ثوابت DEFAULTS جديدة |
| `presentation/tests/engine-test.js` | اختبار لكل دالة قبل تنفيذها (TDD) |
| `presentation/athar-prototype.html` | عرض النطاقات بشارات مصدر + زر تصدير WZDx + بطاقة الحالة المقابلة |
...
**Files:**
- Modify: `presentation/athar-engine.js` (كتلة `DEFAULTS`، السطر ~33)
...
في `athar-engine.js` داخل كائن `DEFAULTS` بعد سطر `trenchCostPerKmSAR`:
...
**Files:**
- Modify: `presentation/athar-engine.js` (بعد دالة `co2`)
- Test: `presentation/tests/engine-test.js`
...
في `athar-engine.js` بعد دالة `co2()`:
...
**Files:**
- Modify: `presentation/athar-engine.js` (بعد `personHours`)
- Test: `presentation/tests/engine-test.js`
...
**Files:**
- Modify: `presentation/athar-engine.js`
- Test: `presentation/tests/engine-test.js`
...
**Files:**
- Modify: `presentation/athar-prototype.html`

**Interfaces:**
- Consumes: `AtharEngine.personHours` · `timeValueSAR` · `co2Range` · `transitImpact` (Tasks 2–4)، `formatArabicNumber()` القائمة، عنصر `#delayVehHours` القائم (السطر ~199)، دالة `renderScore(result)` القائمة (السطر ~692).
...
1. `node presentation/server.js` ثم فتح `http://localhost:8734/athar-prototype.html` في Browser pane.
...
**Interfaces:**
- Consumes: `AtharEngine.wzdx` (Task 5)، حالة النموذج القائمة `state.lastScoreResult` و`state.selectedCandidate` (المستخدمة سطر ~1049)، إحداثيات مقطع العرض القائمة في كود الخريطة.
...
1. متصفح: نقر الزر ⇒ تنزيل `athar-wzdx-demo.geojson`.
2. `node -e "const f=require('C:/Users/wasan/Downloads/athar-wzdx-demo.geojson');console.log(f.features[0].properties.vehicle_impact)"` أو فتح الملف يدوياً: `vehicle_impact` صحيح.
...
**Interfaces:**
- Consumes: `AtharEngine.predictionError` (Task 6)، `AtharEngine.backTest` القائمة، عدّاد back-test القائم (grep عن `backTest(` في الملف لتحديد المشهد).

- [ ] **Step 1: HTML — داخل مشهد back-test بعد عنصر العدّاد القائم**

grep أولاً: `backTest` في `athar-prototype.html` لتحديد الحاوية. يُضاف بعد عنصر العدّاد:
...
- [ ] **Step 2:** فحص الوسوم: `grep -c "افتراض توضيحي" presentation/athar-engine.js` ⇒ العدد ازداد عن الأصل (كل ثابت جديد موسوم).
- [ ] **Step 3:** فحص الممنوعات: `grep -nE "10,?000|١٠٬٠٠٠|627" presentation/athar-engine.js presentation/athar-prototype.html` ⇒ لا نتائج.
...
- [ ] **Step 5:** التأكد أن ملفات ChatGPT لم تُلمس: `git status` غير متاح (لا مستودع) — بديل: مقارنة تواريخ التعديل `ls -l presentation/athar-pitch.html presentation/athar-merged.html presentation/athar.html` ⇒ لم تتغير أثناء التنفيذ.
