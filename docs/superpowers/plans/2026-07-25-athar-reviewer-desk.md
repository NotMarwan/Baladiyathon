...
**Architecture:** لا نعيد كتابة أي محرك. كل المحركات الحالية (`athar-engine`, `athar-routing`, `athar-forecast`, `athar-conflict`, `athar-reasons`, `athar-decision`, `athar-portfolio`) تبقى كما هي وتُستهلَك من سطح واحد جديد: `athar-desk.html` — ثلاثة أعمدة RTL (صندوق الأعمال · الخريطة · ملف القرار) فوق مخزن حالة واحد. الوصلة المفقودة الكبرى: محفظة الـ١٥٠ تصريحاً موجودة بلا مكان، والخريطة موجودة بلا محفظة — ندمجهما.

**Tech Stack:** HTML/CSS/JS عادي بلا إطار وبلا حزم. UMD بنمط `athar-engine.js`. اختبارات `node assert`. الخريطة MapLibre GL 5 محلية. لا شبكة وقت التشغيل.
...
- عقد الـ API الحالي لخريطة الأعمال (١٤ دالة) لا يُكسر. `athar-prototype.html` يستهلكه.
...
| `athar-decision.html` | ١١٩٦ سطراً، ١٩ اختباراً ناجحاً، **صفر روابط واردة** | أقوى صفحة في المشروع غير قابلة للوصول |
...
| محفظة `athar-portfolio.js` | ١٥٠ تصريحاً **بلا هندسة**، بأسماء وهمية «شرياني أ» | أرقام بلا مكان |
...
|---|---|
| `presentation/athar-tokens.css` | مصدر الحقيقة البصري: متغيّرات اللون والمسافة والزاوية والكثافة والتوقيت. لا قاعدة مكوّن هنا. |
| `presentation/scripts/build-city-portfolio.js` | يُشغَّل مرة. يدمج `athar-portfolio.buildPermits()` مع محاور `riyadh-roads.geojson` المسمّاة وينتج هندسة مثبّتة على الإسفلت. |
| `presentation/data/city-portfolio.geojson` | ناتج البناء: ~١٥٠ تصريحاً بمخطط `WorkPermit` كامل. |
| `presentation/data/city-portfolio.geojson.js` | النسخة العالمية (`window.ATHAR_CITY_PORTFOLIO`) للتشغيل من القرص. |
| `presentation/athar-desk-store.js` | مخزن الحالة: التحديد، المرشحات، الفرز، الاشتراكات. نقي. |
| `presentation/athar-desk-states.js` | آلة حالة العمل + الحُرّاس + الإجراء التالي + سجل التدقيق. نقي. |
| `presentation/athar-desk-inbox.js` | تصيير صندوق الأعمال (صف العمل، الوسوم، الفرز). نقي في `render`. |
| `presentation/athar-desk-file.js` | تصيير ملف القرار (التبويبات، بطاقة القرار، شريط الثقة). نقي في `render`. |
| `presentation/athar-desk.html` | التركيب: ثلاثة أعمدة + ربط المخزن بالخريطة واللوحتين. |
| `presentation/athar-desk.css` | تنسيق المكتب فوق `athar-tokens.css`. |
...
| الملف | التعديل |
|---|---|
| `presentation/athar-nav.js` | إعادة تلوين فاتحة من الوسوم + «مكتب المراجع» صفحةً أولى + إظهار `athar-decision.html`. |
| `presentation/athar-map.html` | إطار الشفافية العامة: بحث بالشارع + آخر تحديث + المحفظة الكاملة بدل ١٦. |
| `presentation/athar-worksmap.js` | إضافة `highlightWork(id)` و`onWorkHover(cb)` إلى العقد (إضافة فقط — لا كسر). |
| `presentation/athar-worksmap-page.css` | استهلاك `athar-tokens.css` بدل قيمه الخاصة. |
| `presentation/tests/nav-test.js` | ست صفحات بدل أربع. |
| `presentation/README-athar.md` | جدول الصفحات ومسار المحكّم. |
...
**Files:**
- Create: `presentation/athar-tokens.css`
- Create: `presentation/tests/tokens-test.js`
- Modify: `presentation/athar-worksmap-page.css`
- Modify: `presentation/athar-nav.js:18-27`

**Interfaces:**
- Produces: متغيّرات CSS بالبادئة `--athar-*` تستهلكها كل الصفحات اللاحقة. لا تصدّر جافاسكربت.
...
المتوقع: `ENOENT ... athar-tokens.css`.

- [ ] **Step 3: اكتب ملف الوسوم**

`presentation/athar-tokens.css` — القيم مأخوذة حرفياً من `design-and-interaction-system.md` (هوية أثر، ليست لوحة منافس):

```css
/* أثر — الوسوم البصرية. مصدر الحقيقة الوحيد للون والمسافة والتوقيت.
...
:root {
  ...
  --athar-radius-sm: 6px;
  --athar-radius: 10px;
  --athar-radius-lg: 14px;
  --athar-radius-pill: 999px;

  /* كثافة وضع المراجع */
  --athar-row-h: 56px;
  --athar-toolbar-h: 48px;
  --athar-panel-w: 384px;
  --athar-text: 14px;

  /* التوقيت */
  --athar-t-hover: 100ms;
  --athar-t-control: 160ms;
  --athar-t-panel: 240ms;
  --athar-t-map: 300ms;
  --athar-ease: cubic-bezier(0.16, 1, 0.3, 1);

  /* الظل */
  --athar-shadow-sm: 0 1px 2px rgb(21 32 43 / 0.06);
  --athar-shadow: 0 4px 16px rgb(21 32 43 / 0.10);
  --athar-shadow-lg: 0 12px 32px rgb(21 32 43 / 0.14);

  /* الخطوط */
  --athar-font: "Noto Sans Arabic", "Segoe UI", Tahoma, Arial, sans-serif;
  --athar-font-mono: ui-monospace, "Cascadia Code", Consolas, monospace;
}
...
- [ ] **Step 4: حوّل `athar-worksmap-page.css` إلى الوسوم**

استبدل كتلة `:root` في أعلى الملف بسطر استيراد وربط:

```css
@import url("athar-tokens.css");

:root {
  --wm-surface: var(--athar-surface);
  --wm-border: var(--athar-line);
  --wm-stage: var(--athar-canvas);
  --wm-text: var(--athar-ink);
  --wm-muted: var(--athar-muted);
  --wm-accent: var(--athar-primary);
  --wm-radius: var(--athar-radius);
  --wm-shadow: var(--athar-shadow);
  --wm-bar: 52px;
}
```

وفي `.wm-badge` استبدل الألوان الخام بـ `var(--athar-accent)` و`var(--athar-accent-soft)` و`var(--athar-line)`، وفي `.wm-error` بـ `var(--athar-danger)` و`var(--athar-danger-soft)`.

- [ ] **Step 5: أعد تلوين شريط التنقل**

في `athar-nav.js` استبدل نص `style.textContent` بالتالي، وأضف حقن الوسوم قبله:
...
```bash
git add presentation/athar-tokens.css presentation/athar-worksmap-page.css presentation/athar-nav.js presentation/tests/tokens-test.js
...
**Files:**
- Modify: `presentation/athar-nav.js:9-14`
- Modify: `presentation/athar-decision.html` (وسم `<script src="athar-nav.js">` + استيراد الوسوم)
- Modify: `presentation/tests/nav-test.js`

**Interfaces:**
- Consumes: `athar-tokens.css` من المهمة ١.
- Produces: ترتيب الصفحات النهائي الذي تعتمده المهمة ١١.

السبب: `athar-decision.html` فيها ١٩ اختباراً ناجحاً وبوابة جودة بيانات وامتناع عن التوصية عند النقص — وهي **أقرب شيء في المستودع إلى ما يطلبه المحكّم**، ولا يوجد إليها رابط واحد. هذه أرخص نقطة في الخطة كلها.
...
المتوقع: `nav missing link to athar-desk.html`.

- [ ] **Step 3: حدّث `PAGES` في `athar-nav.js`**
...
- [ ] **Step 4: ضع مؤقتاً `athar-desk.html` كتحويل**
...
<head><meta charset="utf-8"><title>أثر — مكتب المراجع</title>
<link rel="stylesheet" href="athar-tokens.css">
<style>body{font-family:var(--athar-font);background:var(--athar-canvas);
color:var(--athar-ink);display:grid;place-items:center;min-height:100vh;margin:0}</style></head>
<body><p>مكتب المراجع قيد التركيب — افتح <a href="athar-decision.html">شاشة القرار</a>.</p>
<script src="athar-nav.js"></script></body></html>
```

- [ ] **Step 5: أضف الشريط والوسوم إلى `athar-decision.html`**
...
```bash
git add presentation/athar-nav.js presentation/athar-decision.html presentation/athar-desk.html presentation/tests/nav-test.js
...
**Interfaces:**
- Consumes: `athar-portfolio.buildPermits()` (١٥٠ تصريحاً، بذرة `20260727`)، `scripts/lib/centreline.js` (`chainForRoad`, `section`, `lengthOf`)، `data/riyadh-roads.geojson`.
- Produces: `window.ATHAR_CITY_PORTFOLIO` — `FeatureCollection` بخصائص `WorkPermit` التالية بالضبط:
...
`presentation/scripts/build-city-portfolio.js`. المبدأ: البذرة نفسها في `athar-portfolio.js` تقود كل اختيار، فالناتج قابل لإعادة الإنتاج بالضبط. الأثر يُحسب من `AtharEngine.score()` — **ملاحظة حرجة: `score()` لا يملك قيماً افتراضية داخلية، فيجب تمرير `capacityPerLane` و`freeFlowMin` صراحة من `AtharEngine.DEFAULTS`.**
 ...
 *
 * 1) نفس بذرة athar-portfolio: الناتج قابل لإعادة الإنتاج حرفياً.
...
fs.writeFileSync(outJs, 'window.ATHAR_CITY_PORTFOLIO = ' + text + ';');
...
**Files:**
- Modify: `presentation/athar-map.html:36` (مصدر البيانات) و`:66-99` (الربط)
- Modify: `presentation/tests/worksmap-page-test.js`

**Interfaces:**
- Consumes: `window.ATHAR_CITY_PORTFOLIO` من المهمة ٣.
...
في `athar-map.html` استبدل السطر:
...
```js
    var works = window.ATHAR_WORKS_CITY || { type: 'FeatureCollection', features: [] };
```
...
شغّل `node presentation/server.js` وافتح `http://localhost:8734/athar-map.html`. تحقق من: ظهور تجميعات بعدّادات ثنائية الرقم، عدم تحوّل الخريطة إلى شبكة صمّاء، وأداء التحريك سلساً. إن ثقُلت، أضف `minzoom: 10` إلى طبقات الخطوط في `athar-worksmap-layers.js`.

- [ ] **Step 6: Commit**

```bash
git add presentation/athar-map.html presentation/tests/worksmap-page-test.js
...
**Files:**
- Create: `presentation/athar-desk-store.js`
...
المتوقع: `Cannot find module ... athar-desk-store.js`.

- [ ] **Step 3: اكتب المخزن**

`presentation/athar-desk-store.js`:
...
```bash
git add presentation/athar-desk-store.js presentation/tests/desk-store-test.js
...
**Files:**
- Create: `presentation/athar-desk-states.js`
...
`presentation/athar-desk-states.js`:
...
```bash
git add presentation/athar-desk-states.js presentation/tests/desk-states-test.js
...
**Files:**
- Create: `presentation/athar-desk-inbox.js`
- Create: `presentation/athar-desk-file.js`
- Create: `presentation/tests/desk-render-test.js`

**Interfaces:**
- Consumes: `AtharDeskStates.LABELS`، `AtharDeskStates.nextAction`.
- Produces:
...
- [ ] **Step 3: اكتب `athar-desk-inbox.js`**

المواصفة الملزِمة (الاختبار يفرضها؛ الشكل الداخلي حر ما دام يمر):

- `escapeHtml` مشتركة بنفس شكلها في `athar-worksmap-interactions.js`.
- `renderRow` يخرج `<li role="option" data-work-id … aria-selected …>` يحتوي: المرجع (بخط أحادي)، الشارع، نافذة التنفيذ (تاريخان مختصران)، الأثر (`impactVehHours` + `delayPct`)، الثقة، وسم الحالة (`<span data-tone="…"><span aria-hidden="true">الرمز</span> النص</span>`)، والإجراء التالي من `AtharDeskStates.nextAction`.
- ارتفاع الصف من `--athar-row-h`.
- كل قيمة غائبة تصبح `—`.
...
- [ ] **Step 4: اكتب `athar-desk-file.js`**
...
- `renderSummary(feature, analysis)` → بطاقة القرار: التوصية، الفرق عن الطلب، أكبر ثلاثة أسباب من `AtharReasons.explain`، نطاق الثقة، مصدر البيانات، وزرّا «اعتماد» و«إرجاع».
...
```bash
git add presentation/athar-desk-inbox.js presentation/athar-desk-file.js presentation/tests/desk-render-test.js
...
**Files:**
- Create: `presentation/athar-desk.css`
- Modify: `presentation/athar-desk.html` (استبدال الصفحة الانتقالية بالتركيب الكامل)
- Modify: `presentation/athar-worksmap.js` (إضافة `highlightWork` و`onWorkHover` — إضافة فقط)
- Modify: `presentation/tests/worksmap-api-test.js` (العقد صار ١٦ دالة)

**Interfaces:**
- Consumes: `AtharDeskStore`, `AtharDeskStates`, `AtharDeskInbox`, `AtharDeskFile`, `AtharWorksMap`, `AtharEngine`, `AtharReasons`, `AtharConflict`, `window.ATHAR_CITY_PORTFOLIO`.
- Produces: `window.__atharDesk = { store, map, api }` لأغراض الفحص الآلي.
...
في `athar-worksmap.js` أضف إلى `API_METHODS`:
...
و`highlightWork(id)` يضبط مصدر `work-highlight` على هندسة العمل ويطير إليه بـ `--athar-t-map`؛ `onWorkHover(cb)` يسجّل مستمعاً على `mousemove` فوق طبقات الأعمال ويبلّغ بالمعرّف أو `null`.

- [ ] **Step 2: حدّث عقد الاختبار**

في `tests/worksmap-api-test.js` أضف الاسمين إلى قائمة الدوال المتوقعة وارفع العدد إلى ١٦.

- [ ] **Step 3: اكتب `athar-desk.css`**

يستورد `athar-tokens.css` ثم: شبكة ثلاثة أعمدة، صفوف بارتفاع `--athar-row-h`، تمرير داخلي للعمودين الجانبيين فقط، `:hover` على الصف بـ `--athar-primary-soft`، `:focus-visible` بحلقة `--athar-primary`، وسوم الحالة تقرأ `data-tone` وتخرّج لونها من `--athar-{tone}` و`--athar-{tone}-soft`. عند `max-width: 1100px` تنطوي الأعمدة إلى تبويبات. `prefers-reduced-motion` يوقف كل انتقال.

- [ ] **Step 4: اكتب `athar-desk.html`**

الربط الملزم:

1. المخزن يُنشأ من `AtharWorksMapData.normalizeWorks(window.ATHAR_CITY_PORTFOLIO).features` بعد دمج خصائص المحفظة الأصلية (التطبيع يُبقي `status`/`nextAction` عبر تمريرهما في `properties`).
...
7. زر «اعتماد» يستدعي `AtharDeskStates.guard`؛ عند وجود عوائق تُعرض عبر `File.renderBlockers` **ولا يُنفَّذ الإجراء**؛ عند الخلو يُستدعى `apply` ويُدفَع الحدث إلى قائمة تدقيق مرئية في تبويب «التاريخ».
...
ثم في المتصفح على `http://localhost:8734/athar-desk.html` تحقق من: `console` نظيفة، صندوق يفتح على عمل حالته `StrategyReview`، النقر يطير بالخريطة، الملف يمتلئ، «اعتماد» على عمل ناقص المدخلات يعرض العائق ولا يغيّر الحالة.

- [ ] **Step 6: Commit**

```bash
git add presentation/athar-desk.html presentation/athar-desk.css presentation/athar-worksmap.js presentation/tests/worksmap-api-test.js
...
**Files:**
- Modify: `presentation/athar-desk.html` (وصل المحركات)
- Modify: `presentation/athar-desk-file.js` (`renderSummary` بمدخلات حقيقية)
- Modify: `presentation/tests/desk-render-test.js` (+٤ اختبارات)

**Interfaces:**
- Consumes: `AtharEngine.score`, `AtharEngine.optimize`, `AtharReasons.explain`, `AtharConflict.analyze`.
...
في `athar-desk.html`، دالة `analyze(feature)` تُستدعى عند كل تحديد:
...
```js
  function analyze(feature) {
    ...
    var alternatives = AtharEngine.optimize(input).slice(0, 3);
    var best = alternatives[0];
    return {
      scored: scored,
      alternatives: alternatives,
      reasons: AtharReasons.explain(input, scored).slice(0, 3),
...
`overlapping(feature)` يبحث في المخزن عن أعمال على الشارع نفسه بنافذة زمنية متقاطعة ويمررها إلى `AtharConflict.analyze`.
...
```bash
git add presentation/athar-desk.html presentation/athar-desk-file.js presentation/tests/desk-render-test.js
...
**Files:**
- Modify: `presentation/athar-map.html`
...
في الشريط العلوي لـ `athar-map.html`:
...
```bash
git add presentation/athar-map.html presentation/athar-worksmap-page.css presentation/tests/worksmap-page-test.js
...
- Modify: `presentation/README-athar.md`
...
`presentation/tests/judge-walkthrough-test.js` — فحص ثابت على `athar-desk.html` وملفاته، سبع خطوات:
...
ok('٣ — يرى الأثر والثقة: المحرك وشريط الثقة موصولان', () => {
  assert.ok(desk.indexOf('AtharEngine') !== -1);
...
ok('٥ — يفهم سبب التوصية: الأسباب الرقمية معروضة', () => {
  assert.ok(desk.indexOf('AtharReasons') !== -1);
});
...
ok('لا إعادة إدخال: المكتب يقرأ المحفظة ولا يعرض نموذج إدخال يدوي', () => {
  assert.ok(desk.indexOf('ATHAR_CITY_PORTFOLIO') !== -1);
});
...
أضف في أعلى `README-athar.md` مسار المحكّم الجديد:
...
```bash
git add presentation/tests/judge-walkthrough-test.js presentation/tests/run-all.js presentation/README-athar.md
