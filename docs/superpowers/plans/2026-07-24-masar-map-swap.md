...
**Goal:** استبدال `masar-glmap.js` و `masar-ownedmap.js` بوحدة خريطة واحدة `masar-worksmap.js` تحمل لغة خريطة one.network البصرية (ثلاثية casing/line/symbol، شرطات، تجميع، لوحة طبقات) مع الحفاظ الحرفي على عقد الـ API الذي يعتمد عليه `masar-prototype.html`، وبقاء الخريطة تعمل بلا أي طلب شبكة خارجي.

**Architecture:** وحدة UMD واحدة بنفس نمط `masar-engine.js`، تبني style JSON يدوياً فوق مصادر GeoJSON محلية (`riyadh-roads.geojson` + `riyadh-base.geojson`)، وتضيف طبقات الأعمال من `works.geojson` عبر مصنع الثلاثية. الخطوط والأيقونات وإضافة RTL كلها مستضافة محلياً تحت `vendor/`. الدوال البانية نقية وتُختبر في Node بلا متصفح، تماماً كـ `glmap-test.js` اليوم.
...
| الحقيقة | الدليل |
|---|---|
| الخريطة الحالية وحدتان | `masar-glmap.js` (MapLibre، 17KB) و `masar-ownedmap.js` (Leaflet Canvas، 15KB) |
| النموذج يحمّل الاثنتين | `masar-prototype.html:441-445` |
| GL هو المسار الأساسي، Leaflet احتياطي | `masar-prototype.html:698-701, 741-746` |
...
| أسماء الطرق عناصر DOM | `collectLabelAnchors` في `masar-glmap.js` |
...
`masar-prototype.html` يستدعي **١٤ دالة** عبر `GL.api` بالإضافة إلى `GL.map` الخام. أي وحدة بديلة تكسر واحدة منها تكسر النموذج:
...
ومواضع استخدام `GL.map` الخام (لا تمر عبر api): `masar-prototype.html:772-804` (مصدر `deco` وطبقاته وبوب-أب)، `857` (Marker)، `1581, 1889, 1899`.
...
- UMD بنفس نمط `masar-engine.js` — يعمل في Node (`module.exports`) وفي المتصفح (`root.MasarWorksMap`).
...
- كل ملف ≤ 400 سطر. `masar-worksmap.js` يتجاوزها ⇒ يُقسم كما في جدول الملفات أدناه.
...
├── masar-worksmap.js          ← جديد. init() + عقد api الكامل (≤ 400 سطر)
├── masar-worksmap-style.js    ← جديد. buildStyle() للخريطة الأساسية الفاتحة (نقي)
├── masar-worksmap-layers.js   ← جديد. buildTriple/buildWorksLayers/الفلاتر/التجميع (نقي)
├── masar-worksmap-data.js     ← جديد. normalizeWorks() محوّل works.geojson (نقي)
├── masar-map.css              ← تعديل. تنسيق لوحة الطبقات والبوب-أب
├── masar-prototype.html       ← تعديل. سطور 441-445 و 696-746 و 772-804
├── data/
...
**Files:**
- Create: `Baladiyathon/presentation/masar-worksmap-style.js`
...
- Produces:
  ```
  MasarWorksMapStyle.BASE_COLORS   // كائن ألوان الخريطة الأساسية
  MasarWorksMapStyle.roadWidth(scale)      → تعبير عرض حسب التقريب والتصنيف
  MasarWorksMapStyle.buildStyle(roads, base, options) → style JSON
  ```
...
Expected: FAIL — `Cannot find module '.../masar-worksmap-style.js'`
...
```bash
git add presentation/masar-worksmap-style.js presentation/tests/worksmap-style-test.js
...
**Files:**
- Create: `Baladiyathon/presentation/masar-worksmap-layers.js`
...
- Produces:
  ```
  MasarWorksMapLayers.WORKS_COLORS
  MasarWorksMapLayers.LINE_WIDTH / CASING_WIDTH        // تعبيرات عرض
  MasarWorksMapLayers.dashByZoom(pattern)              → تعبير step
  MasarWorksMapLayers.LAYER_GROUPS                     // 5 مجموعات
  MasarWorksMapLayers.buildTriple(config)              → [casing, line, symbol]
  MasarWorksMapLayers.buildWorksLayers(binding)        → كل الطبقات
  MasarWorksMapLayers.buildClusterLayers(source)       → [circle, count]
  MasarWorksMapLayers.CLUSTER_OPTIONS
  MasarWorksMapLayers.baseFilters()                    → { layerId: filter }
  MasarWorksMapLayers.buildDateFilter(range)           → تعبير أو null
  MasarWorksMapLayers.composeFilter(base, extra)       → تعبير
  ```
...
Expected: FAIL — `Cannot find module '.../masar-worksmap-layers.js'`
...
```js
/**
 * مسار — مصنع طبقات الأعمال (نمط one.network)
 * ---------------------------------------------------------------------------
...
(function (root, factory) {
  ...
  if (typeof module === 'object' && module.exports) {
    ...
    root.MasarWorksMapLayers = factory();
  }
})(typeof self !== 'undefined' ? self : this, function () {
  'use strict';
...
```bash
git add presentation/masar-worksmap-layers.js presentation/tests/worksmap-layers-test.js
...
**Files:**
- Create: `Baladiyathon/presentation/masar-worksmap-data.js`
...
- Produces:
  ```
  MasarWorksMapData.normalizeWorks(raw)  → { type:'FeatureCollection', features:[...] }
  // خصائص كل ميزة: { id, group, subtype, title, description, start_ts, end_ts, severity, promoter, road }
  MasarWorksMapData.splitByGeometry(collection) → { points, lines }
  ```
...
Expected: FAIL — `Cannot find module '.../masar-worksmap-data.js'`
...
```bash
git add presentation/masar-worksmap-data.js presentation/data/works.geojson presentation/tests/worksmap-data-test.js
git commit -m "feat: normalize masar works data into the map schema"
```
...
**Files:**
- Create: `Baladiyathon/presentation/masar-worksmap.js`
- Test: `Baladiyathon/presentation/tests/worksmap-api-test.js`

**Interfaces:**
- Consumes: `MasarWorksMapStyle.buildStyle` (مهمة 3) · `MasarWorksMapLayers.*` (مهمة 4) · `MasarWorksMapData.*` (مهمة 5)
- Produces:
  ```
  MasarWorksMap.init(container, roadsGeoJSON, options) → { map, api }
  MasarWorksMap.API_METHODS  // مصفوفة أسماء العقد — يستخدمها الاختبار
  ```
...
Expected: FAIL — `Cannot find module '.../masar-worksmap.js'`

- [ ] **Step 3: كتابة الوحدة**

`masar-worksmap.js` — الهيكل الكامل:
...
```bash
git add presentation/masar-worksmap.js presentation/tests/worksmap-api-test.js
...
**Files:**
- Modify: `Baladiyathon/presentation/masar-map.css`
- Modify: `Baladiyathon/presentation/masar-prototype.html` (حاوية اللوحة)
- Create: `Baladiyathon/presentation/masar-worksmap-panel.js`
- Test: `Baladiyathon/presentation/tests/worksmap-panel-test.js`

**Interfaces:**
- Consumes: `MasarWorksMapLayers.LAYER_GROUPS` (مهمة 4) · `api.setDateRange` و `api.toggleGroup` (مهمة 6)
- Produces: `MasarWorksMapPanel.render(groups)` → سلسلة HTML · `MasarWorksMapPanel.mount(root, api, nowMs)`
...
Expected: FAIL — `Cannot find module '.../masar-worksmap-panel.js'`
...
```js
/**
 * مسار — لوحة التحكم بالخريطة (الفترة الزمنية + إظهار الطبقات)
 * ---------------------------------------------------------------------------
...
(function (root, factory) {
  ...
  if (typeof module === 'object' && module.exports) {
    ...
    root.MasarWorksMapPanel = factory();
  }
})(typeof self !== 'undefined' ? self : this, function () {
  'use strict';
  ...
  function render(groups) {
    ...
    var toggles = groups.map(function (group) {
      return '<label class="masar-map-toggle">' +
        ...
        '<span class="masar-map-swatch" style="background:' + escapeHtml(group.swatch) + '"></span>' +
        '<span>' + escapeHtml(group.label) + '</span>' +
        '</label>';
    }).join('');

    return '<label class="masar-map-field"><span>الفترة</span>' +
      '<select id="masar-date-preset">' + options + '</select></label>' +
      '<fieldset class="masar-map-field"><legend>الطبقات</legend>' + toggles + '</fieldset>';
  }

  function mount(root, api, nowMs) {
    root.innerHTML = render(require_groups());

    var select = root.querySelector('#masar-date-preset');
    if (select) {
  ...
  function require_groups() {
    ...
    return globalScope.MasarWorksMapLayers.LAYER_GROUPS;
  }
...
في `masar-prototype.html` مباشرة بعد سطر `<div id="map" ...></div>` (السطر 201) أضف:

```html
    <aside id="masar-map-panel" aria-label="عناصر التحكم بالخريطة"></aside>
```

- [ ] **Step 5: إضافة تنسيق اللوحة**

في نهاية `masar-map.css`:
...
.masar-map-field { display: grid; gap: 6px; font-size: 0.83rem; }
.masar-map-field select {
  padding: 6px 9px;
  font: inherit;
  color: inherit;
  background: #ffffff;
...
.masar-map-field select:focus-visible { outline: 2px solid #1971c2; outline-offset: 1px; }
fieldset.masar-map-field { border: 0; padding: 0; }
fieldset.masar-map-field legend { font-weight: 600; margin-bottom: 5px; }
.masar-map-toggle { display: flex; align-items: center; gap: 8px; padding: 2px 0; cursor: pointer; }
.masar-map-toggle input { accent-color: #1971c2; width: 15px; height: 15px; }
.masar-map-swatch { width: 10px; height: 10px; border-radius: 50%; flex: 0 0 auto; }
...
@media (max-width: 640px) {
  #masar-map-panel { inset-inline: 12px; width: auto; }
}
```
...
```bash
git add presentation/masar-worksmap-panel.js presentation/masar-map.css presentation/masar-prototype.html presentation/tests/worksmap-panel-test.js
...
**Files:**
- Modify: `Baladiyathon/presentation/masar-prototype.html` (سطور 441-445 و 696-746)
- Delete: `Baladiyathon/presentation/masar-glmap.js`
- Delete: `Baladiyathon/presentation/masar-ownedmap.js`
...
**Interfaces:**
- Consumes: `MasarWorksMap.init` (مهمة 6) · `MasarWorksMapPanel.mount` (مهمة 7)
...
Expected: FAIL — `masar-glmap.js ما زال موجوداً`

- [ ] **Step 3: استبدال وسوم التحميل**

في `masar-prototype.html` استبدل السطور 441-445:

```html
<script src="vendor/leaflet.js"></script>
<script src="vendor/maplibre-gl.js"></script>

<script src="masar-ownedmap.js"></script>
<script src="masar-glmap.js"></script>
```
...
<script src="masar-worksmap-style.js"></script>
<script src="masar-worksmap-layers.js"></script>
<script src="masar-worksmap-data.js"></script>
<script src="masar-worksmap.js"></script>
<script src="masar-worksmap-panel.js"></script>
```
...
```js
  ...
  fetch('data/riyadh-base.geojson')
    ...
    .then(function (baseGeoJSON) {
      GL = MasarWorksMap.init(document.getElementById('map'), RIYADH_ROADS, {
        scrollZoom: false,
      ...
      GL.api.onReady(function () {
        var panel = document.getElementById('masar-map-panel');
        if (panel) MasarWorksMapPanel.mount(panel, GL.api, Date.now());
...
git rm masar-glmap.js masar-ownedmap.js
...
```bash
git add -A
git commit -m "refactor: swap masar map to the works map and drop leaflet"
```
...
Expected: لا سطر `FAILED:`. أي فشل في `ui-smoke-test.js` أو `server-test.js` بسبب إشارة إلى `masar-glmap.js` أو `leaflet` يُصلَح بتحديث الإشارة إلى `masar-worksmap.js`.
...
**Files:**
- Modify: `Baladiyathon/presentation/README-masar.md`
- Modify: `Baladiyathon/README.md` (إن ذكر الخريطة القديمة)

- [ ] **Step 1: تحديث وصف الخريطة**

في `README-masar.md` استبدل أي ذكر لـ `masar-glmap.js` / `masar-ownedmap.js` بفقرة:

```markdown
## الخريطة

`masar-worksmap.js` — خريطة الأعمال بلغة one.network البصرية فوق بيانات محلية:

| الملف | المسؤولية |
|---|---|
| `masar-worksmap-style.js` | الخريطة الأساسية الفاتحة من GeoJSON محلي |
| `masar-worksmap-layers.js` | ثلاثية casing/line/symbol + التجميع + الفلاتر |
| `masar-worksmap-data.js` | تطبيع `works.geojson` إلى مخطط الرسم |
| `masar-worksmap-panel.js` | لوحة الفترة الزمنية وإظهار الطبقات |
| `masar-worksmap.js` | التهيئة وعقد الـ API الذي يستهلكه النموذج |
...
2. **تحويل بقية الواجهة إلى الأبيض.** الخريطة فاتحة من اللحظة الأولى، لكن `masar-map.css` وبقية النموذج ما زالت داكنة. توحيد الواجهة كلها على الأبيض الرسمي مشروع منفصل بخطة مستقلة — لا يعطّل تبديل الخريطة، والخطوة الوحيدة المطلوبة هنا هي تثبيت خلفية `#map` على الفاتح (مهمة ٧).

3. **طبقة الازدحام المروري.** غير مضمّنة (تكلفة مزوّد + اتصال خارجي يكسر شرط العرض بلا إنترنت).

4. **مصدر بيانات الأعمال الحقيقي.** `masar-worksmap-data.js` يحوّل مخطط أثر الحالي. أي مصدر بلدي جديد = دالة `toCanonical` ثانية في نفس الملف.
...
6. **الرسوم المتحركة السينمائية** (الميلان، موجة الفتح `line-gradient`) الموجودة في `masar-glmap.js` غير منقولة. إن كانت مطلوبة للعرض، تُضاف كطبقة `corridor-glow` مستقلة بعد استقرار التبديل.
...
**فجوة معروفة:** `masar-worksmap.js` يقارب 400 سطر عند اكتماله. إن تجاوزها، اسحب دوال الممر (`corridorFC`, `pushCorridor`, حالات الممر) إلى `masar-worksmap-corridor.js` بنفس نمط UMD.
