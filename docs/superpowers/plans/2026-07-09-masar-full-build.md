...
**Goal:** Ship submission-ready Masar: real computing engine + interactive prototype UI + minimal backend API + 3-min pitch deck + official idea-card text + vault docs.

**Architecture:** Static-first (judge opens one HTML file, everything works offline except map tiles). `masar-engine.js` = pure logic, shared verbatim by browser UI, Node tests, and Node stdlib server. No frameworks, no build step. Leaflet vendored locally (no critical CDN).
...
- Design identity = masar-merged.html tokens, copied verbatim:
  ```css
  :root{
...
File `presentation/masar-engine.js`, UMD-style export (`window.MasarEngine` in browser, `module.exports` in Node):

```js
// All pure functions. No DOM, no fetch, no Date.now dependencies in math.

MasarEngine.HOURLY_PROFILE       // number[24], fractions summing to 1.0 — typical urban weekday distribution (AM peak 7-9, PM peak 16-19)
MasarEngine.DEFAULTS             // {aadt:85000, lanes:4, capacityPerLane:1800, freeFlowMin:6, lengthKm:4.2, valueOfTimeSAR:45, idleFuelLPerHour:0.9, co2KgPerL:2.31, trenchCostPerKmSAR:850000}
                                 // each key documented with a comment: source OR "illustrative demo assumption"

MasarEngine.bprTravelTime(freeFlowMin, volume, capacity) -> minutes
  // t = t0 * (1 + 0.15 * (v/c)^4)  — standard BPR volume-delay function

MasarEngine.score({aadt, lanes, lanesClosed, capacityPerLane, freeFlowMin, lengthKm, startHour, durationHours})
...
MasarEngine.optimize(input) -> {top3: [{label, startHour, phases, delayVehHours, savedVehHours, savedPct, reasons: [string,string,string]}], baseline: {delayVehHours}}
...
MasarEngine.co2(savedVehHours) -> {fuelL, co2Kg}
  // fuelL = savedVehHours * idleFuelLPerHour; co2Kg = fuelL * co2KgPerL (2.31 kg/L gasoline — standard factor, source in comment)

MasarEngine.digOnce({trenchKm, permitsMerged}) -> {separateSAR, sharedSAR, savedSAR, savedPct}
...
MasarEngine.compound(scoreA, scoreB) -> {combined, factor, warning}
...
MasarEngine.backTest(input, chosen) -> {beforeVehHours, afterVehHours}
...
**Files:**
- Create: `presentation/masar-engine.js` (~200 lines, the API above, every constant commented with source-or-assumption)
...
- Create: `presentation/server.js` — Node stdlib only (`http`, `fs`, `path`, `url`). Serves static files from `presentation/` + JSON API:
  - `POST /api/score` → MasarEngine.score(body)
  - `POST /api/optimize` → MasarEngine.optimize(body)
  - `POST /api/digonce` → MasarEngine.digOnce(body)
...
- Create: `presentation/README-masar.md` — Arabic quickstart: (أ) الوضع الثابت: افتح masar-prototype.html مباشرة، (ب) وضع الـ API: `node server.js` ثم نفس الصفحة تكتشف الخادم تلقائيًا، (ج) الاختبارات: `node tests/engine-test.js`, (د) بنية الملفات.

**Acceptance:** `node presentation/tests/engine-test.js` → ALL TESTS PASSED. `node -e "const e=require('./presentation/masar-engine.js'); console.log(JSON.stringify(e.optimize(e.DEFAULTS===undefined?{}:Object.assign({lanesClosed:2,startHour:8,durationHours:48},e.DEFAULTS)).top3[0]))"` runs without error. Server starts, `GET /api/works` returns valid GeoJSON (verify with curl or node fetch, then kill process).
...
- Create: `presentation/masar-prototype.html` — self-contained page (inline CSS/JS except vendored leaflet + masar-engine.js via relative `<script src>`), design tokens verbatim, RTL.

**UI = the 7-step demo script, interactive:**
1. Header: eyebrow «بلدياتثون 2026 · التحدي الثالث», title «مسار — النموذج التفاعلي», demo-corridor badge «طريق الملك فهد (مقطع العليا) — بيانات توضيحية للعرض».
...
**Acceptance:** open file:// directly — zero console errors, all 7 demo steps clickable in order, values change with inputs, night schedule beats day schedule visibly. Then `node server.js` + http://localhost:8734/masar-prototype.html — API badge appears, same behavior. RTL correct, matches design tokens.

## Task W3: Pitch deck + Idea Card (parallel with W1)

**Files:**
- Create: `presentation/masar-pitch.html` — 3-minute deck, ~10 slides, keyboard/click navigation (scroll-snap or arrow keys — inline JS, no libs), same design tokens, RTL. Slides:
  1. مسار — الغلاف (name, tagline from masar-merged header, challenge label)
  ...
  4. الديمو المباشر: لقطة/إطار من masar-prototype.html + الخطوات السبع مختصرة (رابط للنموذج)
...
- `SwarmingObsidian/01 Masar/Prototype.md`, `Engine.md`, `Idea Card.md` — one-paragraph summaries + file links + how to run.
- Link the three from `00 Masar - Home.md` (append section «مخرجات 2026-07-09», don't disturb existing content).
- Append line to `presentation/README-masar.md` if paths changed.
