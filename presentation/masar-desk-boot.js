...
(function () {
   ...
   */
  /* أول صفٍّ في الطابور ليس أوّل ما ينبغي أن يُرى.
     ---------------------------------------------------------------------
     المكتب يفتح على ١٥٠ تصريحاً، والمحكّم ينظر إلى الصفّ المختار وحده قبل أن
     يقرّر إن كان المنتج يستحق دقيقة أخرى. وكان الاختيار `getVisible()[0]` —
     ترتيبَ وصولٍ لا دلالة فيه، فوقع على تصريح **ثقته منخفضة**: أضعف ما في
     المحفظة في أول نظرة.
     والمعيار في `MasarDeskInbox.pickDefault` يُشتقّ من الطابور نفسه ولا يُثبَّت
     بمعرّف: معرّفٌ مكتوب بخطّ اليد يتقادم مع أول إعادة توليد للمحفظة، فيفتح
     الديمو على صفٍّ محذوف أمام اللجنة. */
  function selectInitial() {
    if (store.getState().selectedId) return;
    var first = typeof MasarDeskInbox !== 'undefined' && MasarDeskInbox.pickDefault
      ? MasarDeskInbox.pickDefault(store.getVisible())
      : store.getVisible()[0];
