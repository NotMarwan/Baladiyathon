...
(function () {
   ...
   */
  /* أول صفٍّ ليس أوّل ما ينبغي أن يُرى: `getVisible()[0]` ترتيبُ وصولٍ لا دلالة
     فيه، فكان المكتب يفتح على تصريح **ثقته منخفضة** — أضعف ما في المحفظة في أول
     نظرة. والمعيار في `MasarDeskInbox.pickDefault` يُشتقّ من الطابور ولا يُثبَّت
     بمعرّف يتقادم مع أول إعادة توليد. */
  function selectInitial() {
    if (store.getState().selectedId) return;
    var first = MasarDeskInbox.pickDefault(store.getVisible());
