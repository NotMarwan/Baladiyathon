'use strict';
/**
 * كثافة التقاطعات على مسارات مسار — مؤشر سياق معلَن، لا معامل.
 * ---------------------------------------------------------------------------
 * **السؤال الذي جاء من التحكيم.**
 *
 * سعة الشريان الحضري محكومة بالإشارة لا بالمقطع. ونموذج الحمل في أثر
 * (`edgeLoad` داخل `masar-city-routing.js`) يحسب السعة `عدد الحارات × سعة صنف
...
function main() {
  ...
  const report = {
    ...
    capacityModelInProduct: {
      where: 'masar-city-routing.js — edgeLoad',
