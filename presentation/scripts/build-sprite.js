 ...
 * ---------------------------------------------------------------------------
 * قاعدة الطقم: **الرسم يقول نوع العمل، ولون الشارة يقول المجموعة**.
 * `works-maintenance` و `poi-maintenance` مفتاحُ ربطٍ واحد بلونين — كهرماني
 * لأعمال الطرق وأخضر لنقاط الاهتمام. وبذلك يُقرأ النوع من الشكل والمجموعة من
 * اللون، ويبقى الدليل صادقاً: لون الصف في الدليل هو لون الشارة على الخريطة.
 *
 * أيقونات بلا مستهلك اليوم — تبقى عمداً:
 *   `bus-stop`   : ذهبت معها طبقة الحافلات لأن لا سجل يحمل `subtype: 'bus'`.
 *   `poi-parking`: لا نقطة اهتمام من نوع «مواقف» في المحفظة الحالية.
 *   `works-end`  : «انتهاء الأعمال» حدثٌ لا يصدره مولّد المحفظة.
 * الثلاثة مفردات جاهزة ليوم تصل بيانات نقل عام أو مواقف أو إشعارات انتهاء؛
 * وجودها في الـ atlas لا يَعِد الساكن بشيء — الوعد يقع في عنوان الطبقة، وقد
 * صُحّح. أما الاحتياطي المقصود فاثنان: `roadworks` رسمُ `roadworks/default`،
 * و`poi-information` رسمُ أي نوع نقطةٍ لم يُعرَّف بعد.
 */
const fs = require('fs');
...
// 32 لا 26: الشارة نفسها 26 والباقي مجال نزيف الظل. أي قص أضيق يقطع الظل عند
// حدّ الخلية داخل الـ atlas فيظهر خطاً داكناً حول الأيقونة.
const ICON_SIZE = 32;
const PADDING = 2;
