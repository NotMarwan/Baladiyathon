/**
 * أثر — خطة إدارة المرور: مخرَج ملموس من المكتب.
 * ---------------------------------------------------------------------------
...
(function (root, factory) {
  ...
  function renderTab(plan) {
    ...
    return '<section class="desk-plan">'
      ...
      + 'نزّل WZDx</button>'
      + '</div>'
      /* WP-WZ1 — ضاقت العبارة ولم تُرفع.
         كانت «تصدير تجريبي… لم يجتز بعد مخطط الإصدار 4.2»، وكان ذلك دقيقاً:
         التحقق المعزول رصد إحدى عشرة مخالفة. زالت المخالفات وصار التصدير
         يجتاز `wzdx-schema-test.js`.
         لكن تلك الحزمة **ليست المحقق الرسمي**: قيودها منقولة يدوياً من
         المواصفة، ولا `ajv` ولا نسخة من المخطط في المستودع. فالقول «متوافق»
         يظل ادعاءً يسبق دليله. العبارة تقول ما ثبت بالضبط: بنيةٌ مفحوصة
         داخلياً، ومحققٌ رسميّ لم يُشغَّل. */
      + '<p class="desk-plan-note">الخطة وثيقة واحدة تُفتح وتُطبع بلا برنامج. '
      + 'وWZDx صيغة تبادل لمناطق العمل تقرؤها أنظمة الملاحة — '
      + '<strong>وهذا تصدير على بنية الإصدار 4.2 يجتاز فحصاً بنيوياً داخلياً '
  ...
  function toDocument(plan) {
    ...
    return '<!doctype html>\n<html lang="ar" dir="rtl">\n<head>\n'
      ...
      + 'الأثر من محرك أثر (BPR) على هندسة OpenStreetMap · بيانات توضيحية للعرض.</p>\n'
      + '</body>\n</html>\n';
  }
