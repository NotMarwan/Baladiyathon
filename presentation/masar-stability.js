/**
 * مسار — استقرار التوصية، والامتناع حين لا تستقرّ.
 * ---------------------------------------------------------------------------
...
(function (root, factory) {
  ...
  if (typeof module === 'object' && module.exports) {
    module.exports = factory(require('./masar-sensitivity.js'));
  } else {
    root.MasarStability = factory(root.MasarSensitivity);
  }
  ...
  var DATA_NEEDED = {
    ...
    /* الطلبان مفصولان لأن ما يغلقهما مختلف — وخلطهما كان جوهر العيب:
       السعة تُغلق بعدّاد يقيس **تدفقاً**، والأرضية تُغلق بقياس **زمن رحلة**.
       طلبُ «قياس سعة» لإغلاق أرضية زمن يرسل الجهة تجمع الرقم الخطأ. */
    workZoneLaneCapacity: 'عدّ تدفق على الحارات المفتوحة أثناء إغلاق فعلي في '
      + 'الرياض، بدقة خمس عشرة دقيقة وعلى شريان بإشارات. هذا هو الافتراض '
      + 'الأول الذي يستحق حالةً ميدانية واحدة — وأقرب ما يعادله اليوم قياس '
      + 'أمريكي على نوع الطريق نفسه.',
    workZoneFriction: 'زمن رحلة مقيس عبر منطقة عمل في **ساعة منخفضة الطلب** '
      + 'مقارناً بالمقطع نفسه بلا إغلاق. هذه الأرضية بلا سند مقيس، ولا يغلقها '
      + 'قياس سعة: السعة تقيّد قرب التشبّع، والأرضية تعمل حين لا ازدحام. '
      + 'والسؤال الأكبر قبلها قرارٌ لا قياس: هل تبقى الأرضية أصلاً بعد أن صارت '
      + 'سعة منطقة العمل مقيسة داخل النموذج؟',
