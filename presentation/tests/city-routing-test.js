...
/*
 * الأداء يُقاس بأفضل جولة من خمس، لا بجولة واحدة باردة.
 * ---------------------------------------------------------------------------
 * الصيغة السابقة قاست دفعةً واحدة بـ`Date.now()`. وعلى جهازٍ مشغول تقيس
 * المُجدوِل لا الخوارزمية: قيس التشتّت في **العملية الواحدة** فكان
 * ٢١٧ ⟵ ٤٦٤ مللي — أي أن الجولة الواحدة قد تعطي ضعف الحقيقة. ونتيجته سقوطٌ
 * متكرّر نُسب إلى «تقطّع معروف» في `docs/engineering/FLAKY-SERVER-SUITES-2026-07-26.md`
 * بينما سببه المقياس نفسه.
 *
 * القياس على اثنتي عشرة جولة أعطى: **الأدنى ٢١٧ · الوسيط ٢٩٥ · الأعلى ٣٢٧**.
 * فالحدّ الحقيقي للخوارزمية دون المئتين وخمسين، وما فوقه ضجيج بيئة.
 *
 * **والحدّ لم يُرفع.** ٢٥٠ مللي كما كان — المتغيّر هو الطريقة: أفضل جولة تلغي
 * ذروة جمع المهملات وتبديل السياق، فتقيس الشيفرة. ورفعُ الحدّ كان سيخفي
 * انحداراً حقيقياً؛ وتكرارُ القياس لا يخفيه — انحدارٌ في الخوارزمية يرفع
 * الجولات الخمس كلها.
 *
 * و`hrtime` بدل `Date.now()`: الأخير دقّته عشرات المللي على ويندوز، وهي كسرٌ
 * معتبَر من ميزانية مئتين وخمسين.
 */
ok('الأداء يحتمل الحساب عند فتح البطاقة', () => {
  ...
  assert.ok(best < BUDGET_MS,
    `${best.toFixed(0)} مللي لكل سجل في أفضل جولة من ${ROUNDS} — يُحسّ عند الفتح.\n`
    + `    الجولات: ${rounds.map((value) => value.toFixed(0)).join(' · ')} مللي\n`
    + '    أفضل جولة فوق الحدّ يعني انحداراً في الخوارزمية لا حِملاً على الجهاز.');
});
