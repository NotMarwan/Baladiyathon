...
/*
 * الأداء: بوابة صلبة على رتبة المقدار، وهدفٌ منتَجيّ يُطبع ولا يُسقط.
 * ---------------------------------------------------------------------------
 * الصيغة الأولى قاست دفعةً واحدة بـ`Date.now()` وأسقطت الحزمة عند ٢٥٠ مللي.
 * ثم قيست الحقيقة، وهي أسوأ من «جولة باردة»: **الكلفة نفسها تتحرّك مع حِمل
 * الجهاز بأكثر من الضعف**، بالبيانات والشيفرة والرسم نفسه (118,893 عقدة):
 *
 *   جهاز هادئ    أفضل جولة ٢١٧ · وسيط ٢٩٥
 *   جهاز مشغول   أفضل جولة ٤٠٠–٤٦٧ · وسيط ٤٩٨–٥٦٢
 *
 * وقيس الطرفان في الشجرة الرئيسة والشجرة المعزولة معاً فتطابقا — فالفرق حِمل
 * لا شيفرة. وأفضلُ خمسِ جولات لا يبتلع فارقاً بهذا الحجم؛ جرّب فسقط مرة من
 * ثلاث على جهاز مشغول.
 *
 * والخلاصة أن ٢٥٠ مللي **هدف منتَج** لا حدّ اختبار: هامشه على جهاز هادئ ١٣٪،
 * وهو أضيق من أن يُبنى عليه حكمُ نجاحٍ أو فشل بساعة حائط. وبوابةٌ تسقط ثلث
 * التشغيلات تُعطَّل خلال أسبوع، فتصير صفراً بدل أن تكون قليلاً.
 *
 * فالفصل:
 *   · **البوابة الصلبة** ٧٥٠ مللي — ثلاثة أضعاف الهادئ ونحو ضعفَي المشغول.
 *     لا تسقط على حِمل، وتسقط حتماً على انحدار خوارزمي حقيقي (أول ما يُكسر في
 *     A\* هو الكومة الثنائية، وكسرها يضاعف الكلفة مرات لا نسباً).
 *   · **الهدف المنتَجيّ** ٢٥٠ مللي — يُطبع في كل تشغيل مع الجولات كلها، فيبقى
 *     مرئياً ومقيساً بلا أن يرهن الخط الأخضر بانشغال الجهاز.
 *
 * وهذا ليس تخفيفاً للحدّ: الحدّ الذي كان يُفحص لم يكن يقيس الخوارزمية أصلاً.
 */
ok('الأداء يحتمل الحساب عند فتح البطاقة', () => {
  const ROUNDS = 5;
  const HARD_LIMIT_MS = 750;
  const PRODUCT_TARGET_MS = 250;
  const rounds = [];
  for (let round = 0; round < ROUNDS; round += 1) {
    ...
    rounds.push(Number(process.hrtime.bigint() - started) / 1e6 / 10);
  }
  const best = Math.min(...rounds);
  const shown = rounds.map((value) => value.toFixed(0)).join(' · ');
  console.log(`     أفضل ${best.toFixed(0)} مللي/سجل `
    + `(الهدف ${PRODUCT_TARGET_MS}، البوابة ${HARD_LIMIT_MS}) — الجولات: ${shown}`);
  if (best > PRODUCT_TARGET_MS) {
    console.log(`     تنبيه: فوق الهدف المنتَجيّ بـ${(best - PRODUCT_TARGET_MS).toFixed(0)} مللي`
      + ' — حِملُ جهازٍ أو انحدارٌ حقيقي؛ أعِد القياس على جهاز هادئ.');
  }
  // A\* بكومة ثنائية: الحدّ يبقى كما كان رغم تضاعف الشبكة عشر مرات.
  assert.ok(best < HARD_LIMIT_MS,
    `${best.toFixed(0)} مللي لكل سجل في أفضل جولة من ${ROUNDS} — فوق البوابة الصلبة.\n`
    + `    الجولات: ${shown} مللي\n`
    + '    هذا ليس حِملَ جهاز: البوابة ثلاثة أضعاف الكلفة على جهاز هادئ.\n'
    + '    ابحث عن انحدار في بنية البحث — الكومة الثنائية أولاً.');
});
