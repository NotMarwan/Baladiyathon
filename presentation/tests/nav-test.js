...
const navJs = fs.readFileSync(path.join(ROOT, 'athar-nav.js'), 'utf8');
...
ok('الشريط لا يتجاوز خمسة أقسام', () => {
  const links = navJs.match(/\{\s*file:\s*'athar-[^']+'/g) || [];
...
ok('كل صفحة عائلة تحمّل الشريط', () => {
  family.forEach((page) => {
    ...
    assert.ok(html.includes('src="athar-nav.js"'), `${page} لا يحمّل الشريط`);
  });
});
...
ok('صفحات القسم المتقدم تُعلِّم قسمها في الشريط', () => {
  ...
  const listed = (navJs.match(/'athar-[a-z-]+\.html'/g) || [])
    .map((s) => s.replace(/'/g, ''));
...
ok('قائمة الشريط للمتقدمة تطابق الفهرس — لا فرع يتقادم', () => {
  ...
  const inNav = (block.match(/'athar-[a-z-]+\.html'/g) || []).map((s) => s.replace(/'/g, ''));
...
ok('الشريط يقرأ الوسوم ولا يعرّف لوحته', () => {
  assert.ok(navJs.includes('var(--athar-'), 'الشريط لا يقرأ الوسوم');
});
