 ...
 * ---------------------------------------------------------------------------
 * القاعدة التي يفرضها هذا الملف: **الشريط مسار لا فهرس.** ستة أقسام على
...
ok('الشريط لا يتجاوز ستة أقسام', () => {
  const links = navJs.match(/\{\s*file:\s*'masar-[^']+'/g) || [];
  assert.ok(links.length <= MAX_MAIN,
    `${links.length} قسماً في الشريط — السقف ${MAX_MAIN}`);
});

ok('الأقسام الستة بترتيبها: الرئيسية ثم التجربة ثم الخريطة ثم المكتب ثم النظرة ثم المتقدمة', () => {
  const order = MAIN
...
ok('التسميات عربية والصفحة الحالية معلَّمة', () => {
  ...
  ['الرئيسية', 'تجربة المستخدم', 'الخريطة', 'مكتب المراجع', 'نظرة عامة',
    'التفاصيل المتقدمة']
    .forEach((label) => {
