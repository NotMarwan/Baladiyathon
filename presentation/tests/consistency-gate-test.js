...
const BANNED = [
  ...
  },
  {
    /* صُحِّح: العلّة القديمة كانت «محرك المدينة لا يحمّل الحركة المحوَّلة —
       ...
       فالحظر كان مصوَّباً على الاتجاه المعاكس: يكبح وصفاً **صحيحاً**. بقي
       منه ما لا يزال كاذباً وحده — كلمة «مثبت». التحميل نموذجٌ بقاعدة عرض
       معلنة لا قياس اختيار سائق، فوصفه بالإثبات يسبق دليله. والوصف المجرّد
       («يحمّل»، «مطبَّق»، «يعمل») صار مستحقاً فلا يُحظر.

       الفحص الذي يحرس هذه العلّة الجديدة أسفل الملف: إن زال التحميل من
...
test('محرك المدينة يحمّل الحركة المحوَّلة فعلاً — العلّة المحرَّرة أعلاه مسنودة', () => {
  /* هذا الفحص وُجد لعيبٍ حقيقي: علّةُ حظرٍ في هذا الملف بقيت تقول «صفر
     تطابق» بعد أن صار المحرك يحمّل. الحزمة كانت خضراء والتعليل كاذباً —
     أسوأ من فحص ساقط، لأن الأخضر يُقرأ حراسةً.

     فالعلّة الجديدة لا تُترك نصّاً: تُثبَّت على المحرك نفسه. إن نُزع التحميل
     سقط هذا الفحص وأشار إلى الحظر الذي يجب أن يعود معه. */
  const engine = require(path.join(ROOT, CITY_ENGINE));
  ['divertedDemand', 'loadRoute'].forEach((fn) => {
    assert.strictEqual(typeof engine[fn], 'function',
      `${CITY_ENGINE} لا يصدّر ${fn} — علّة الحظر أعلاه صارت بلا سند`);
  });

  const source = fs.readFileSync(path.join(ROOT, CITY_ENGINE), 'utf8');
  assert.ok(/alternativesAround[\s\S]*?loadRoute\(/.test(source),
    `${CITY_ENGINE} يصدّر loadRoute دون أن يستدعيه من alternativesAround — `
    + 'فالبدائل المعروضة غير محمَّلة، وحكم حمل البديل بلا أساس');

  /* والمستهلك الذي يخرج منه حكم البدائل في العرض: لو انفصل عن هذا المحرك
     لصار الرقم المعروض من مصدر آخر لا يحرسه هذا الفحص. */
  const builder = fs.readFileSync(
    path.join(ROOT, 'scripts', 'build-alternate-load.js'), 'utf8');
  assert.ok(builder.indexOf(CITY_ENGINE) !== -1,
    `build-alternate-load.js لا يستورد ${CITY_ENGINE} — حكم حمل البديل من مصدر غير محروس`);
});
