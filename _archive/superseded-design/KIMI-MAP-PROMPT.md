...
3. **Data**: the road network is a local GeoJSON (`data/riyadh-roads.geojson.js`, exposed as `window.RIYADH_ROADS`, ~1035 LineString features, properties: `osmId, highway (motorway|trunk|primary|secondary|tertiary), name, lanes, aadt`). The module `masar-ownedmap.js` currently renders it. Keep the data contract intact.
...
- **Pixonal "Fusion" for Abu Dhabi Mobility (Red Dot 2025, on mapbox.com/blog BUILD 2025 roundup)** — a government mobility control-room aesthetic: calm, premium, authoritative. This is EXACTLY the emotional register Masar needs — a tool a ministry would buy.
...
1. `masar-ownedmap.js` — rewritten, same exported API: `{ styleFor, toRoutingSegments, load(map, geojson, L, onRoadClick), CLASS_AADT, CLASS_LANES }`. `toRoutingSegments` output shape must not change (routing depends on it).
2. A `<style>` block (or `masar-map.css`) with all map-specific CSS (panes, glows, animations, controls, RTL tooltips).
...
- `presentation/masar-ownedmap.js` — current module (rewrite this)
- `presentation/masar-prototype.html` — host page (map div `#map`, init around line 687; see how `MasarOwnedMap.load` is called)
- `presentation/data/riyadh-roads.geojson.js` — the network data
- `presentation/masar-merged.html` — design tokens of the product family
