---
tags: [masar, research, capabilities]
---
...
- Before approving a closure, Masar checks it doesn't sever the fastest ambulance/civil-defense corridor to nearby hospitals; flags and reroutes if it does.
...
- When Masar co-locates competing digs in one trench, it also quantifies the shared-trench saving vs separate excavations, and offers the open trench to fiber/utilities.
...
- A citizen-facing live map of active/upcoming permits with impact rating and dates — the same data Masar publishes to nav apps, made public.
...
- On approval, Masar can push the closure + Arabic TMP summary to citizens via existing Saudi gov channels rather than a new app.
